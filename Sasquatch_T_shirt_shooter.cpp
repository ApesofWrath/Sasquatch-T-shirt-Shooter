...
void enabled()
{
  ...
  {
    leftPower = constrain(255 - (usb1.rightY() - usb1.rightX() + 127), 0, 255);
    rightPower = constrain(255 - (usb1.rightY() + usb1.rightX() - 127), 0, 255);
  }
  ...
  if (PRESSURIZE || !atPressure())
  {
    if (SHUTOFF)
    {
      RODashboard.debug("Either you have exceeded the maximum tank Pressure or the pressure switch is not connected (to Digital IO 1)");
    }
  ...
  val = 127;
  if (usb1.dPadUp())   val = (float)(127.0 + (LIFT_SPEED * 128.0));
  if (usb1.dPadDown()) val = (float)(127.0 - (LIFT_SPEED * 128.0));
